...
BOOL BuildPageForPrint(PrintParam_t *P, WORD &Dup, bool &isLastPage, bool &NeedInit )
{
    ...
    while ( P->BlkCur ) 
    {
        ...
        prevBlk = P->BlkCur;
        P->BlkCur = P->BlkCur->Next;
        // if (DBG_MODE) LogAddLine("Set current block <%s>. Is last page %i", (P->BlkCur == NULL)? "EMPTY" : "Not empty", isLastPage);
    ...
    // ВНИМАНИЕ!!! Для послендней страницы текущий блок NULL в процессе обработки цикла.
    // Чтобы этого избежать выставляем в конце цикла текущий блок как предыдущий
    // Ошибка проявляется как не возможность указания ориентации последней страницы
    if (P->BlkCur == NULL)
        P->BlkCur = prevBlk;
